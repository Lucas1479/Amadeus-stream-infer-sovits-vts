...
**Embodied Real-time AI Agent · 具身实时 AI Agent**

[![English](https://img.shields.io/badge/English-README-blue)](#english) &nbsp;|&nbsp; [![中文](https://img.shields.io/badge/中文-README-blue)](#中文版)
...
- **Optional CUDA Graph acceleration**: static KV cache, 210–260 it/s throughput (see [Performance](#performance-en))
...
| CUDA Graph | Optional static KV cache; 210–260 it/s (RTX 4070 Ti Super) |
...
> Measured on **RTX 4070 Ti Super**, prompt length ~330–400 tokens.

#### CUDA Graph Acceleration (AR Decoder)

CUDA Graph captures the autoregressive decoding loop as a static GPU program, eliminating per-step CPU dispatch overhead. Combined with stride-aligned static KV cache and startup pre-capture, this delivers:

| Metric | Without CUDA Graph | With CUDA Graph (`ENABLE_CUDA_GRAPH=1`) |
|---|---|---|
| First-sentence latency | ~1.5–2.0 s | **~0.8 s** |
| Inference throughput | 100–140 it/s | **210–260 it/s** |
| Startup overhead | 0 | +~2 s (one-time pre-capture) |

#### Configuration

| Scenario | Config | Notes |
|---|---|---|
| High-throughput (single turn, long text) | `ENABLE_CUDA_GRAPH=1` + `EXP_TTS_MAX_CONCURRENCY=1` | 210–260 it/s, serialized |
| Low-latency (rapid multi-sentence) | `ENABLE_CUDA_GRAPH=0` + `EXP_TTS_MAX_CONCURRENCY=2` | 100–140 it/s, concurrent |

To limit which KV buckets are pre-captured at startup (reduces startup time):

```env
ENABLE_CUDA_GRAPH=1
CUDA_GRAPH_PRECAPTURE_BUCKETS=512   # only pre-capture the bucket actually used
```
...
- **CUDA Graph 可选加速**：开启后使用静态 KV 缓存，推理吞吐提升至 210–260 it/s（见[性能](#性能)）
...
| CUDA Graph 加速 | 可选开启，静态 KV 缓存，210–260 it/s（RTX 4070 Ti Super） |
...
> 以下数据在 **RTX 4070 Ti Super** 上实测，prompt 长度约 330–400 token。

#### CUDA Graph 加速（AR 解码器）

CUDA Graph 将自回归解码循环录制为静态 GPU 程序，消除每步的 CPU dispatch 开销。结合步长对齐的静态 KV 缓存与启动时预捕获，效果如下：

| 指标 | 不开 CUDA Graph | 开启 CUDA Graph（`ENABLE_CUDA_GRAPH=1`） |
|---|---|---|
| 首句 TTS 延迟 | ~1.5–2.0 s | **~0.8 s** |
| 推理吞吐 | 100–140 it/s | **210–260 it/s** |
| 程序启动额外开销 | 0 | +~2 s（一次性预捕获） |

#### 配置建议

| 场景 | 配置 | 备注 |
|---|---|---|
| 高吞吐（单轮长句） | `ENABLE_CUDA_GRAPH=1` + `EXP_TTS_MAX_CONCURRENCY=1` | 210–260 it/s，串行 |
...
```env
ENABLE_CUDA_GRAPH=1
CUDA_GRAPH_PRECAPTURE_BUCKETS=512   # 只预捕获实际用到的 bucket
```
